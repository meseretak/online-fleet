...
@Component({
  selector: 'app-delegate-gsauth',
  templateUrl: './delegate-gsauth.component.html',
  styleUrls: ['./delegate-gsauth.component.css']
})
export class DelegateGsauthComponent implements OnInit {
  ...
  public getUserOnDataTable(): void {
    this.authService.getMechanicUsers().subscribe((ret: UserType[]) => {
      this.data = ret;
      setTimeout(() => {
