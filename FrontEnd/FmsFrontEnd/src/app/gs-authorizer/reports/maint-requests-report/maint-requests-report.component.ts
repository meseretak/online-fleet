...
import { AuthMaintReport } from 'src/app/types/AuthMaintReport';

@Component({
  selector: 'app-maint-requests-report',
  templateUrl: './maint-requests-report.component.html',
  styleUrls: ['./maint-requests-report.component.css'],
})
export class MaintRequestsReportComponent implements OnInit {
  form!: FormGroup;
  requestsReport: any = [];
  report: AuthMaintReport = new AuthMaintReport();
  hasData: Boolean = false;
  noData: boolean = false;
  noDatamessage: string;
  myDate: Date;
  fileName = 'MaintenanceRequests.xlsx';
  cValue: any;
  ...
  public getRequestsOnDataTable(report: AuthMaintReport): void {
    this.gsService.getRequests(report).subscribe(
  ...
  printPDF() {
    ...
    doc.setFontSize(10);
    doc.setTextColor(20);
    doc.text(this.report.listing + ' Maintenance Requests', 170, 17, {
      baseline: 'middle',
    });
    ...
    doc.save(this.report.listing + 'MaintenanceReport');
  }
