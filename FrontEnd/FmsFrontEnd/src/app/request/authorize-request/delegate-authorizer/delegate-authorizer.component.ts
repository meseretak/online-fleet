...
@Component({
  selector: 'app-delegate-authorizer',
  templateUrl: './delegate-authorizer.component.html',
  styleUrls: ['./delegate-authorizer.component.css']
})
export class DelegateAuthorizerComponent implements OnInit {
  ...
  public getUserOnDataTable(): void {
    this.authService.getUsers(sessionStorage.getItem("directorate")).subscribe((ret: UserType[]) => {
      this.data = ret;
      setTimeout(() => {
