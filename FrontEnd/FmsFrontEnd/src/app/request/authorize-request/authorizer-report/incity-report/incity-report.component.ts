...
@Component({
  selector: 'app-incity-report',
  templateUrl: './incity-report.component.html',
  styleUrls: ['./incity-report.component.css'],
})
export class IncityReportComponent implements OnInit {
  requestsReport: any = [];
  ...
  public getRequestsOnDataTable(report: AuthReqReport): void {
    this.authService.getIncityReport(report).subscribe(
      (ret: InCityRequest[]) => {
  ...
  printPDF() {
    ...
    doc.setFontSize(10);
    doc.setTextColor(20);
    doc.text(this.report.listing + ' Incity Requests', 170, 17, {
      baseline: 'middle',
    });
    ...
    doc.save(this.report.listing + 'IncityReport');
  }
