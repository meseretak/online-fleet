...
@Component({
  selector: 'app-offtime-report',
  templateUrl: './offtime-report.component.html',
  styleUrls: ['./offtime-report.component.css'],
})
export class OfftimeReportComponent implements OnInit {
  requestsReport: any = [];
  ...
  public getRequestsOnDataTable(report: AuthReqReport): void {
    this.authService.getOfftimeReport(report).subscribe(
      (ret: OffTimeRequest[]) => {
  ...
  printPDF() {
    ...
    doc.setFontSize(10);
    doc.setTextColor(20);
    doc.text(this.report.listing + ' Offtime Requests', 170, 17, {
      baseline: 'middle',
    });
    ...
    doc.save(this.report.listing + 'OfftimeReport');
  }
