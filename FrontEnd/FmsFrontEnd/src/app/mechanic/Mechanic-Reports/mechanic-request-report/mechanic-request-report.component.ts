...
@Component({
  selector: 'app-mechanic-request-report',
  templateUrl: './mechanic-request-report.component.html',
  styleUrls: ['./mechanic-request-report.component.css']
})
export class MechanicRequestReportComponent implements OnInit {
  ...
  fileName = this.request.mechanicReport+'MechanicRequests.xlsx';
  ...
  viewMechanic(request:Report){
      this.isOnView = true;
      this.mechanicService.getReportOfMechanicRequests(request).subscribe(
  ...
  printPDF() {
    ...
    doc.setFontSize(10);
    doc.setTextColor(20);
    doc.text(this.request.mechanicReport+" Mechanics' Maintenance Requests", 130, 17, { baseline: 'middle' });
    ...
    doc.save(this.request.mechanicReport+'MechanicRequestReport');
  }
