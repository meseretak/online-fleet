...
@Component({
  selector: 'app-custodian-requests',
  templateUrl: './custodian-requests.component.html',
  styleUrls: ['./custodian-requests.component.css']
})
export class CustodianRequestsComponent implements OnInit {
  director: string;
  username: string;
  ...
  ngOnInit(): void {
    if(sessionStorage.getItem("role")=="GS Mechanic"){
      ...
      if(sessionStorage.getItem("verified")!=null){
        this.alert.sucessAlert(sessionStorage.getItem("verified"));
     ...
     this.getCustodianRequests();
      ...
      this.router.navigate(['/home']);
              ...
              public getCustodianRequests():void{
                this.mechanicService.getAllCustodianRequests().subscribe((ret:RequestMaintenance[])=>{
        ...
        //check milege not less than previous
                checkMilege(current:any){
                  if(current.target.value<=this.modifyForm.currentMilage){
            ...
            // request.modifiedBy = this.username;
             this.mechanicService.verifyCustodianRequests(request.value).subscribe(
              (ret:RequestMaintenance)=>{
                sessionStorage.setItem("verified", "Request Successfully Verified");
